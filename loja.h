...
Lista_DE* criar_lista_de_ingredientes ()
{
	...
	for(int i=0;i<QTD_INGREDIENTES;i++){
		...
		somar_cor_obj(Item.titulo, COLOR_PRETO);
		Obj ret;
		...
		somar_cor_obj(Item.preco, COLOR_PRETO);
...
void iniciar_loja(Jogador *J){
	...
	print_rgb_txt(NULL, COLOR_CIANO, nv2(-1, -1), "Bem vindo à loja!\n");
	...
	while(true){
		...
		mostrar_grana(J);
		int qtd_ing_atual = busca(J->estoque, (tp_item_arvore){p->info.ing.id, 0})->info.qtd;
		print_rgb_txt(NULL, qtd_ing_atual == 0 ? COLOR_VERMELHO : COLOR_VERDE, nv2(54, 19), "Qtd no seu estoque: %dx", qtd_ing_atual);
